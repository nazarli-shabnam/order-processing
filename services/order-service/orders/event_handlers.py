...
def handle_order_created(event_data: dict) -> bool:
    ...
    try:
        ...
        logger.info(f"Processing OrderCreated event for order {event.order_id}")
        ...
        if not success:
            logger.error(f"Failed to publish OrderStatusUpdated event for order {order.order_id}")
